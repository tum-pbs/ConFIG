...
class ConFIG(WeightMethod):
    ...
    def _set_config_grads(self, losses, shared_parameters, task_specific_parameters=None):
        # shared part
        shared_grads = []
        for l in losses:
            shared_grads.append(
        ...
        if isinstance(shared_parameters, torch.Tensor):
            shared_parameters = [shared_parameters]
        non_conflict_shared_grads = self._process_grads(shared_grads)
        for p, g in zip(shared_parameters, non_conflict_shared_grads):
            p.grad = g
    ...
    def _process_grads(self, grads: List[Tuple[torch.Tensor]]):
        template_grad = copy.deepcopy(grads[0])
        grad_vecs=torch.stack([self._grad_tensor2vector(g_i) for g_i in grads],0)
        merged_grad = self._grad_vector2tensor(template_grad,self.config_operator.calculate_gradient(grad_vecs))
        return merged_grad
            
    def _grad_tensor2vector(self,grads):
        with torch.no_grad():
            para_vec = None
            for par in grads:
    ...
    def _grad_vector2tensor(self,template_grad,target_grad):
        with torch.no_grad():
            start=0
            for par in template_grad:
                end=start+par.data.view(-1).shape[0]
                par.data=target_grad[start:end].view(par.data.shape)
                start=end
            return template_grad
    ...
    def backward(
        ...
        **kwargs,
        ...
        self._set_config_grads(losses, shared_parameters, task_specific_parameters)
        return None, {}  # NOTE: to align with all other weight methods

class MConFIG(ConFIG):
    
    def __init__(self, n_tasks: int, device: torch.device, reduction="sum",num_updates=1):
        super().__init__(n_tasks, device=device)
        self.momentum_operator=PseudoMomentumOperator(num_vectors=n_tasks,
                                                   gradient_operator=self.config_operator)
    ...
    def _set_config_grads(self, losses, shared_parameters, task_specific_parameters=None):
        # shared part
        shared_grads = []
        indexes,selected_losses=self.loss_selector.select(self.num_updates,losses)
        ...
        template_grad = copy.deepcopy(shared_grads[0])
        grad_vecs=[self._grad_tensor2vector(g_i) for g_i in shared_grads]
        non_conflict_shared_grads = self._grad_vector2tensor(template_grad,
                                               self.momentum_operator.calculate_gradient(indexes,grad_vecs))
        for p, g in zip(shared_parameters, non_conflict_shared_grads):
            p.grad = g
