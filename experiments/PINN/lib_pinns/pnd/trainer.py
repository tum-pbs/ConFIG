import torch
from .data_sampler import PNdSampler,PNdValidationDataLoader,PNdValidationDataSet
...
class PNdTrainerBasis():
    ...
    def set_configs_type_dataloader(self):
        ...
        self.configs_handler.add_config_item("n_dim",mandatory=False,value_type=int,
                                             description="End of x axis",default_value=N_DIM)
    
    def generate_dataloader(self, train_dataset, validation_dataset):
        train_dataloader=PNdSampler(n_internal=self.configs.n_internal,
        ...
        if validation_dataset is not None:
            vali_dataloader=PNdValidationDataLoader(validation_dataset,
                                             device=self.configs.device)
        else:
    ...
    def validation_step(self, network, batched_data, idx_batch: int, num_batches: int, idx_epoch: int, num_epoch: int):  
        return ((self.validate_dataloader.u-network(self.validate_dataloader.x))**2).mean()
    
    def get_internal_loss(self,network,idx_epoch:int):
        x_internal= self.train_dataloader.sample_internal()
        internal_loss=physical_residual(network(x_internal),x_internal).pow(2).mean()
    ...
    def get_bound_loss(self,network,idx_epoch:int): 
        x_b,u_b=self.train_dataloader.sample_boundary()
        bound_loss=torch.mean((network(x_b)-u_b)**2)
...
# Parent classes
class StandardTrainer(PNdTrainerBasis,StandardTrainerBasis):
    pass

def training(epochs,trainer:PNdTrainerBasis,
             random_seeds:Sequence,
             path_config_file,name,num_run=3,
             device="cuda:0",n_validation_point=3125,**kwargs):
    errors=[]
    for i in range(num_run):
        ...
        trainer.train_from_scratch(
            network=network,
            train_dataset=None,
            validation_dataset=PNdValidationDataSet(n_point=n_validation_point),
            run_in_silence=True,
