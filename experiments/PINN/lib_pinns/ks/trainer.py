...
from .data_sampler import KSSampler,KSValidationDataSet,KSValidationDataLoader
...
class KSTrainerBasis():
    ...
    def set_configs_type_dataloader(self):
        ...
        self.configs_handler.add_config_item("simulation_time",mandatory=True,value_type=float,description="simulation time of ks equation")
        
    def generate_dataloader(self, train_dataset, validation_dataset):
        train_dataloader=KSSampler(n_internal=self.configs.n_internal,n_initial=self.configs.n_initial,n_boundary=self.configs.n_boundary,device=self.configs.device,
        ...
        if validation_dataset is not None:
            vali_dataloader=KSValidationDataLoader(validation_dataset,device=self.configs.device)
        else:
    ...
    def get_bound_ini_loss(self,network,idx_epoch:int):
        ...
        """
        boundary_loss=((network(x_left,t_left)-network(x_right,t_right))**2).mean()
        self.recorder.add_scalar("separated_loss/boundary_loss",boundary_loss.item(),idx_epoch)
        """
        u_loss,f_loss=squared_residual_boundary(network(x_left,t_left),network(x_right,t_right),x_left,x_right)
        self.recorder.add_scalar("separated_loss/u_loss",u_loss.item(),idx_epoch)
        self.recorder.add_scalar("separated_loss/f_loss",f_loss.item(),idx_epoch)
        boundary_loss=u_loss+f_loss
        ...
        bound_ini_loss=boundary_loss+initial_loss
...
class StandardTrainer(KSTrainerBasis,StandardTrainerBasis):
    pass

def training(epochs,trainer:KSTrainerBasis,
             random_seeds:Sequence,path_config_file,name,num_run=3,device="cuda:0",**kwargs):    
    simulation_path=package_path()+"data/ks/simulation_data.npy"
    ...
    for i in range(num_run):
        ...
        trainer.train_from_scratch(
            network=network,
            train_dataset=None,
            validation_dataset=KSValidationDataSet(dataset_path=simulation_path),
            run_in_silence=True,
